...
export default function Hero() {
  ...
  return (
    <section id="hero" ref={heroRef}
      ...
      {/* Animated orbs */}
      <div className="absolute top-1/4 left-1/3 w-[500px] h-[500px] bg-primary/15 rounded-full blur-[80px] animate-pulse" style={{animationDuration:"4s"}} />
      <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-secondary/10 rounded-full blur-[80px] animate-pulse" style={{animationDuration:"6s",animationDelay:"2s"}} />
      ...
      <div className="relative z-10 text-center px-4 sm:px-6 max-w-5xl mx-auto w-full">
        ...
        <div className="anim-fade-up delay-100">
          <span className="inline-flex items-center gap-2 px-5 py-2 rounded-full
            bg-white/10 backdrop-blur-md border border-white/20 text-white/90 text-sm font-medium mb-8">
            <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
            Buka Sekarang &nbsp;·&nbsp; 10.00 – 22.00 WIB
          </span>
        </div>

        {/* Headline */}
        <h1 className="anim-fade-up delay-200 text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-black text-white leading-[0.9] mb-5 tracking-tight">
          Pesisir
          <br />
        ...
        {/* Subtext */}
        <p className="anim-fade-up delay-300 text-base sm:text-lg md:text-xl text-white/70
        ...
        {/* CTA Buttons */}
        <div className="anim-fade-up delay-400 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center mb-14">
          <a href="#menu"
          ...
          <a
            ...
            className="btn btn-lg rounded-2xl font-bold border-white/30 text-white
              bg-[#25D366]/80 hover:bg-[#25D366] hover:scale-105 hover:shadow-2xl
              hover:shadow-[#25D366]/30 transition-all duration-300 gap-2">
        ...
        {/* Stats */}
        <div className="anim-scale-in delay-500
          grid grid-cols-2 sm:grid-cols-4 gap-px bg-white/10 backdrop-blur-md
          rounded-2xl overflow-hidden border border-white/15 max-w-2xl mx-auto">
          {STATS.map((s, i) => (
            <div key={i}
              className="flex flex-col items-center py-5 px-3 bg-black/20
                hover:bg-primary/20 transition-colors duration-300 cursor-default">
              <span className="text-xl mb-1">{s.emoji}</span>
              <span className="text-2xl sm:text-3xl font-black text-white">{s.val}</span>
              <span className="text-white/50 text-xs mt-0.5 font-medium">{s.label}</span>
            </div>
          ))}
        </div>
      </div>
