...
function MenuCard({ item, index }: { item: MenuItem; index: number }) {
  ...
  const isLocalImage = item.image?.startsWith("/");

  return (
    <div
      className="card-hover anim-fade-up bg-base-100 rounded-2xl overflow-hidden border border-base-200 group flex flex-col"
      ...
      <div className="relative h-44 overflow-hidden bg-base-200">
        ...
        {item.badge && (
          <span className={`absolute top-3 right-3 text-xs font-bold px-2.5 py-1 rounded-full shadow-lg ${BADGE_STYLE[item.badge] || "bg-primary text-primary-content"}`}>
            {item.badge}
          </span>
        )}
      </div>
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          <button
            ...
            className={`btn btn-sm rounded-xl font-bold transition-all duration-300 gap-1.5
              ${ordered ? "btn-success scale-95" : "btn-primary hover:scale-105 hover:shadow-lg hover:shadow-primary/25"}`}>
            {ordered ? <>✓ Dipesan!</> : (
              <>
...
export default function MenuSection({ standalone = false }: { standalone?: boolean }) {
  ...
  const fetchMenu = useCallback(async () => {
    try {
      ...
      console.error("Gagal fetch menu:", e);
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <section id="menu" className={`${standalone ? "pt-32" : "py-24"} pb-24 bg-base-200 relative overflow-hidden`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative">
        <div className="text-center mb-14 anim-fade-up">
          <span className="inline-block text-primary font-bold tracking-[0.25em] uppercase text-xs sm:text-sm mb-4 px-4 py-1.5 rounded-full bg-primary/10 border border-primary/20">
            Menu Kami
          </span>
        ...
        <div className="flex justify-end mb-4">
          <a href="/admin"
            className="flex items-center gap-2 px-4 py-2 rounded-xl border border-dashed border-base-content/20
              text-base-content/40 hover:border-primary hover:text-primary hover:bg-primary/5
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center mb-6 max-w-lg">
          <div className="relative flex-1">
            <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-base-content/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
            </svg>
            <input type="text" placeholder="Cari menu..." value={q} onChange={e => setQ(e.target.value)}
              className="input input-bordered w-full rounded-xl pl-10 text-sm bg-base-100 focus:border-primary transition-colors" />
          </div>
          {q && (
            <button onClick={() => setQ("")} className="btn btn-ghost btn-sm rounded-xl text-base-content/50 shrink-0">
              ✕ Hapus
            </button>
          )}
        </div>
        ...
        {q && <p className="text-sm text-base-content/40 mb-6">{filtered.length} menu ditemukan untuk &ldquo;{q}&rdquo;</p>}
        ...
        {loading ? (
          ...
          <div className="text-center py-24">
            ...
            <button onClick={() => { setQ(""); setCat("Semua"); }} className="btn btn-ghost btn-sm mt-4 rounded-xl text-primary">
              Reset Filter
            </button>
          </div>
        ) : (
