...
export default memo(function Navbar() {
  ...
  const active = seafoodThemes.find(t => t.name === current) ?? seafoodThemes[0];
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
          ...
          <a href="#hero" className="flex items-center gap-2.5 group shrink-0">
            ...
            <span className="font-black text-lg tracking-tight leading-none">
              ...
              <span className={scrolled || isLight ? "text-base-content" : "text-white"}> Seafood</span>
            </span>
          </a>
              ...
              <li key={l.href}>
                <a href={l.href}
                  className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200
                    ...
                    ${scrolled || isLight ? "text-base-content/75" : "text-white/85"}`}>
                  {l.label}
                </a>
              </li>
            ))}
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="relative" ref={themeRef}>
              <button
                ...
                className={`btn btn-ghost btn-sm gap-1.5 rounded-xl border transition-colors px-2.5
                  ${scrolled || isLight ? "border-base-300 hover:border-primary text-base-content" : "border-white/30 hover:border-white/60 text-white"}`}>
              ...
              {themeOpen && (
                <div className="absolute right-0 top-full mt-2 z-[999] p-1.5 bg-base-100 border border-base-200
                  rounded-2xl shadow-2xl shadow-black/20 w-36 flex flex-col gap-0.5">
                  {seafoodThemes.map(t => (
                    <button
                      key={t.name}
            ...
            <button
              ...
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24"
                stroke={scrolled || isLight ? "currentColor" : "white"}>
                {menuOpen
      ...
      {/* Floating WA */}
      <a
        href={`https://wa.me/${WA_NUMBER}?text=${encodeURIComponent("Halo Pesisir Seafood! 🦞")}`}
        target="_blank" rel="noopener noreferrer"
        aria-label="Chat via WhatsApp"
        style={{ position: "fixed", bottom: "24px", right: "24px", zIndex: 9999 }}
        className="w-14 h-14 rounded-full bg-[#25D366] shadow-2xl shadow-[#25D366]/40
          flex items-center justify-center hover:scale-110 transition-all duration-300 group">
        <span
