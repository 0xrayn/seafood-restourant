...
export default function Navbar() {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 nav-glass
        ${scrolled
          ? "bg-base-100/85 shadow-xl shadow-black/10 py-0"
          : "bg-transparent py-1"
        }`}>

        {/* Top bar — only when NOT scrolled */}
        <div className={`overflow-hidden transition-all duration-500 ${scrolled ? "max-h-0 opacity-0" : "max-h-8 opacity-100"}`}>
          <div className="bg-primary/90 text-primary-content text-xs py-1 text-center font-medium tracking-wide flex items-center justify-center gap-2">
            <span className="inline-block w-1.5 h-1.5 rounded-full bg-green-300 animate-pulse" />
            Buka Sekarang &nbsp;·&nbsp; 10.00 – 22.00 WIB &nbsp;·&nbsp; Jl. Mayangan No.45, Probolinggo
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
          ...
          <a href="#hero" className="flex items-center gap-2.5 group shrink-0">
            ...
            <span className="font-black text-lg tracking-tight leading-none">
              ...
              <span className="text-base-content"> Seafood</span>
            </span>
          </a>
          ...
          <ul className="hidden md:flex items-center gap-0.5">
            {links.map(l => (
              <li key={l.href}>
                <a href={l.href}
                  className="px-4 py-2 rounded-xl text-sm font-semibold text-base-content/70
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="dropdown dropdown-end">
              <button tabIndex={0}
                className="btn btn-ghost btn-sm gap-1.5 rounded-xl border border-base-300 hover:border-primary transition-colors px-2.5">
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-300 nav-glass
          ...
          <div className="px-4 py-4 flex flex-col gap-1 bg-base-100/90">
            {links.map(l => (
